...
public class PetAlert {
    private static final Logger logger = LogManager.getLogger(PetAlert.class);
    private static final String VOICE_SENDONCE_URL = "https://open.ys7.com/api/lapp/voice/sendonce";
    
    private String accessToken;
    private String deviceSerial;
    
    public PetAlert(String accessToken, String deviceSerial) {
    ...
    public Map<String, Object> sendVoiceAlertFromLocal(String voiceFilePath, int channelNo) {
        ...
        try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
            ...
            logger.info("发送宠物告警语音: 设备" + deviceSerial + ", 文件" + voiceFilePath);
            var response = httpClient.execute(httpPost);
            String responseBody = EntityUtils.toString(response.getEntity());
            
            logger.info("宠物告警语音下发响应: " + responseBody);
    ...
    public boolean playPetAlert(String localFilePath) {
        try {
            ...
            logger.info("准备播放宠物告警语音: 设备" + deviceSerial + ", 文件" + localFilePath);
            
            // 直接使用本地文件发送
            Map<String, Object> result = sendVoiceAlertFromLocal(localFilePath, 1);
            
            if (result.containsKey("error")) {
                logger.error("宠物告警语音播报失败: " + result.get("error"));
                return false;
            }
            
            String code = result.get("code").toString();
            if ("200".equals(code)) {
                logger.info("宠物告警语音播报成功");
                return true;
            } else {
                ...
                logger.error("宠物告警语音播报失败: " + errorMsg);
                return false;
            }
            
        } catch (Exception e) {
            logger.error("宠物告警语音播报异常: " + e.getMessage(), e);
            return false;
        }
    }
}