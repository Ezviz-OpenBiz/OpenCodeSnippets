...
public class PetDetector {
    ...
    public Map<String, Object> detectPet(String imageUrl) {
        try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
            ...
            requestBody.put("taskType", "pet_detection");
            ...
            logger.info("执行宠物识别: 图片类型=" + (isLocalFile ? "本地文件" : "URL"));
            ...
            logger.info("宠物识别响应状态码: " + statusCode);
            logger.info("宠物识别响应: " + responseBody);
            
            // 处理404错误（API不存在）
            if (statusCode == 404) {
                Map<String, Object> errorResult = new HashMap<>();
                errorResult.put("is_pet_detected", false);
                errorResult.put("pet_status", "宠物识别API未找到（404），系统使用默认逻辑处理");
            ...
            if (meta != null) {
                ...
                if (code != 200) {
                    ...
                    logger.warn("宠物识别接口错误: " + errorMsg);
                    
                    // 处理API错误但不抛出异常，返回默认结果
                    Map<String, Object> errorResult = new HashMap<>();
                    errorResult.put("is_pet_detected", false);
                    errorResult.put("pet_status", "识别接口错误: " + errorMsg);
                    errorResult.put("confidence", 0.0);
                    errorResult.put("api_error", errorMsg);
                    return errorResult;
                }
            } else {
                logger.warn("宠物识别接口响应格式错误");
                
                // 处理格式错误但不抛出异常，返回默认结果
                Map<String, Object> errorResult = new HashMap<>();
                errorResult.put("is_pet_detected", false);
                errorResult.put("pet_status", "识别响应格式错误");
                errorResult.put("confidence", 0.0);
            ...
            if (images == null || images.isEmpty()) {
                ...
                detectionResult.put("is_pet_detected", false);
                detectionResult.put("pet_status", "未检测到宠物");
            ...
            Map<String, Object> detectionResult = new HashMap<>();
            if (isPetDetected && bestConfidence > 0.5) {
                detectionResult.put("is_pet_detected", true);
                detectionResult.put("pet_status", "检测到宠物");
                detectionResult.put("confidence", bestConfidence);
            } else {
                detectionResult.put("is_pet_detected", false);
                detectionResult.put("pet_status", "未检测到宠物或置信度低");
                detectionResult.put("confidence", 0.0);
            }
            ...
            logger.error("宠物识别异常: " + e.getMessage(), e);
            Map<String, Object> errorResult = new HashMap<>();
            errorResult.put("is_pet_detected", false);
            errorResult.put("pet_status", "识别失败: " + e.getMessage());
            errorResult.put("confidence", 0.0);
            return errorResult;
        }
    }
